...
server.tool(
  "get_template_info",
  "Returns likely fields needed for a specific template.",
  { templateId: z.string() },
  async ({ templateId }) => {
    // Basic heuristic since actual fields are dynamic in typescript functions
    let fields = ["subject", "style", "mood", "colors", "avoid"];
    if (templateId === "clothing") fields.push("background");
    if (templateId === "marketing") fields = ["product", "headline", "style", "mood", "colors"];
    if (templateId === "brand") fields = ["brandname", "subject", "composition"];
    if (templateId === "jewelry") fields = ["piece", "material", "gemstones"];
...
server.tool(
  "list_generators",
  "Lists all supported prompt AI generators and their nuances.",
  {},
  async () => {
    return {
...
server.tool(
  "format_raw_prompt",
  "Formats an explicitly raw positive and negative text for a specific AI generator's optimal syntax.",
  { generatorId: z.string(), positive: z.string(), negative: z.string().optional(), parameters: z.string().optional() },
  async ({ generatorId, positive, negative, parameters }) => {
...
server.tool(
  ...
  "Predicts the internal fields of a provided raw image prompt so it can be re-built using the builder engine.",
  { rawPrompt: z.string() },
  async ({ rawPrompt }) => {
...
server.tool(
  "get_style_modifiers",
  "Returns a list of popular aesthetic styles available.",
  {},
  async () => {
...
server.tool(
  "get_color_palettes",
  "Returns common color palettes for image generation.",
  {},
  async () => {
