...
export function RecipeCard({ recipe }: RecipeCardProps) {
  ...
  return (
    <motion.div
      ...
      className="group bg-bg-2 border border-accent/8 p-4 hover:border-accent/30 transition-colors"
    >
      {/* Header */}
      ...
      <div className="flex flex-wrap gap-1.5 mb-3">
        {recipe.styles.length > 0 && (
          <span className="inline-flex items-center gap-1 px-2 py-0.5 bg-accent/5 border border-accent/10 text-[10px] font-mono text-accent/80">
      ...
      <div className="flex items-center gap-2 pt-2 border-t border-accent/8">
        <button
          onClick={handleLoad}
          className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-mono font-medium bg-accent text-black hover:bg-accent/90 transition-colors"
        >
        ...
        <button
          ...
          className={cn(
            ...
            confirmDelete
              ? "text-red-500 bg-red-500/10 font-mono font-medium px-2"
